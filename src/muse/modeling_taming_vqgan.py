...
class VectorQuantizer(nn.Module):
    ...
    def forward(self, hidden_states, return_loss=False):
        ...
        min_encoding_indices = min_encoding_indices.reshape(hidden_states.shape[0], hidden_states.shape[1], hidden_states.shape[2])
...
class VQGANModel(nn.Module):
    def __init__(
        ...
        self.config = OmegaConf.create(dict(
            resolution=resolution, num_channels=num_channels, hidden_channels=hidden_channels,
            channel_mult=channel_mult, num_res_blocks=num_res_blocks, attn_resolutions=attn_resolutions,
            no_attn_mid_block=no_attn_mid_block, z_channels=z_channels, num_embeddings=num_embeddings,
            quantized_embed_dim=quantized_embed_dim, dropout=dropout, resample_with_conv=resample_with_conv,
    ...
    def init_from_ckpt(self, path, ignore_keys=list()):
        ...
        print(f"[INFO] In VQGAN model, loading checkpoint...")
