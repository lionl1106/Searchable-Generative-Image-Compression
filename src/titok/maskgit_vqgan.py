...
class Decoder(nn.Module):
    ...
    def forward_with_latent(self, hidden_states):
        # z to block_in
        hidden_states = self.conv_in(hidden_states)
        ...
        # end
        hidden_for_out = hidden_states
        hidden_states = self.norm_out(hidden_states)
        hidden_states = F.silu(hidden_states)
        img_for_out = self.conv_out(hidden_states)

        return img_for_out, hidden_for_out
...
class VectorQuantizer(nn.Module):
    ...
    def __init__(self, num_embeddings, embedding_dim, commitment_cost, sane_index_shape=False):
        r"""
        Args:
    ...
    def forward(self, hidden_states, return_loss=False):
        ...
        if self.sane_index_shape:
            min_encoding_indices = min_encoding_indices.reshape(hidden_states.shape[:-1])
        else:
